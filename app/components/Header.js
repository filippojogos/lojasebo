...
export default function Header() {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    // Simulate quick login via Context
    login({ name: "João Pipo", email: "pipo@sebo.com" });
    setIsLoginOpen(false);
  };
                ...
                {mounted && user ? (
                  ...
                  <div className="logged-out-view">
                    ...
                    <form onSubmit={handleLogin} id="quick-login">
                      <input type="email" placeholder="E-mail / Login" required />
                      <input type="password" placeholder="Senha" required />
                      <button type="submit" className="btn-cta btn-block" style={{ marginTop: '10px' }}>Entrar (Teste)</button>
                    </form>
              ...
              {cat.subcategories && (
                <div className="category-dropdown">
                  {cat.subcategories.map(sub => (
                    <Link key={sub.label} href={sub.href} className="subcategory-link">
                      {sub.label}
                    </Link>
                  ))}
                </div>
